...
impl LeveledCompactionController {
    ...
    pub fn generate_compaction_task(
        ...
        let mut target_level_sizes = (0..self.options.max_levels)
            .map(|_| 0)
            .collect::<Vec<_>>();
        ...
        target_level_sizes[self.options.max_levels - 1] = std::cmp::max( 
            actual_level_sizes[self.options.max_levels - 1],base_level_size
        );

        let mut base_level = self.options.max_levels; 
        for level in (0..(self.options.max_levels - 1)).rev() { 
            let next_level = level + 1 ; 
            let this_level_size = target_level_sizes[next_level] / self.options.level_size_multiplier ; 
        ...
        let (ratio, top_priority_level) =  priorities
            .iter()
        ...
        let sstable_to_compact_id = snapshot
            .levels[top_priority_level - 1].1.iter().min().copied().unwrap() ; 
    ...
    pub fn apply_compaction_result(
        ...
        match task.upper_level { 
            Some(upper_level) => { 
                snapshot.levels[upper_level - 1].1 = snapshot
                    .levels[upper_level - 1].1
                    .iter()
                    .copied()
        ...
        snapshot.levels[task.lower_level - 1].1 = snapshot
            .levels[task.lower_level - 1].1
            .iter()
            .copied()
